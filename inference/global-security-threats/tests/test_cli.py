...
`threat_structure` is what a reader runs to reproduce the numbers in the README, so a
...
from threat_structure.cli import main


class TestItRuns:
    def test_the_default_invocation_succeeds(self, capsys):
        assert main(["--draws", "20"]) == 0
        assert capsys.readouterr().out.strip()

    @pytest.mark.parametrize("section", ['structure', 'clusters', 'anomalies'])
    def test_each_section_runs_on_its_own(self, section, capsys):
        assert main([*["--draws", "20"], "--section", section]) == 0
        assert capsys.readouterr().out.strip()
